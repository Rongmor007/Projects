				...
				<script type="text/javascript">
					...
					**/
					...
					expandAll = function()
					{
						...
						var rootImgElem = document.getElementsByClassName("expandCollapseAll");	
						var src = rootImgElem[0].src;
						var imgPath = src.slice(0, src.lastIndexOf('/') + 1);
						rootImgElem[0].src = imgPath + "button_collapse.gif";
						gExpandCollapseState="expanded";
					}
			...
			<tr>
				...
				<td>
					<xsl:choose>
						...
						<xsl:otherwise>
							<span style="color:{$statusColor}">
								<xsl:variable name="status" select="tr:ResultSet/tr:Outcome/@value"/>
								<xsl:choose>
									<xsl:when test="$status='NotStarted'">Skipped</xsl:when>
		...
		<xsl:if test="$endingLoopIndex">
			<div class="expandCollapse" style="left:{$indentation}px;">
				...
				<div class="{$expandCollapseState}" style="left:0px;">
					...
					<xsl:variable name="currentStepIndex" select="tr:Extension/ts:TSStepProperties/ts:Index/@value"/>
					<xsl:for-each select="following-sibling::*[position() &lt;= $endingPositionValue]">
						<xsl:if test="(tr:Extension/ts:TSStepProperties/ts:LoopingProperties/ts:LoopIndex) and ($currentStepIndex = tr:Extension/ts:TSStepProperties/ts:Index/@value)">
							<xsl:apply-templates select=".">
								<xsl:with-param name="reportOptions" select="$reportOptions"/>
								<xsl:with-param name="labelBgColor" select="$labelBgColor"/>
								<xsl:with-param name="valueBgColor" select="$valueBgColor"/>
	...
	<xsl:template name="ReportError">
		...
		<tr>
			<td colspan="2" style="background-color:{$reportOptions/c:Item[@name='Colors']/c:Collection/c:Item[@name='ErrorBg']}">
				<xsl:if test="$eventsNode/tr:Event[@ID='Error Message']">Error Message: <xsl:variable name="isValueEscaped" select="$eventsNode/tr:Event[@ID='Error Message']/tr:Data/c:Datum/@isEscaped = 'true'"/> <xsl:value-of disable-output-escaping="yes" select="user:ReplaceWhitespaceAndNewLine($eventsNode/tr:Event[@ID='Error Message']/tr:Data/c:Datum/c:Value, true(), $isValueEscaped)"/>
				</xsl:if>
	...
	<xsl:template name="GetStepResultImage">
		...
		<xsl:variable name="imageSrc">
			<xsl:choose>
				...
				<xsl:when test="$outcome='NotStarted'">
