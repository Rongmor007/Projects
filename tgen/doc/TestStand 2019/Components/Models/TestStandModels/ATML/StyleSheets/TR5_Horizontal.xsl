<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:trc="urn:IEEE-1636.1:2011:01:TestResultsCollection" xmlns:tr="urn:IEEE-1636.1:2011:01:TestResults" xmlns:c="urn:IEEE-1671:2010:Common" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ts="www.ni.com/TestStand/ATMLTestResults/2.0" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="http://www.ni.com/TestStand" id="TS19.0.0">
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<xsl:value-of select="user:GetPathsOfGraphControlScripts(//trc:TestResults/tr:TestProgram/tr:Configuration/c:Collection/c:Item[@name='StylesheetPath'])" disable-output-escaping="yes"/>
			</head>
					...
					<td colspan="2" style="color:{$reportOptions/c:Item[@name='Colors']/c:Collection/c:Item[@name='Error']}; white-space:normal; text-align:left;">
						<xsl:if test="tr:ResultSet/tr:Events/tr:Event[@ID='Error Message']">Error: 
							<xsl:call-template name="ReplaceNewLineWithBreak">
								<xsl:with-param name="inputString" select="tr:ResultSet/tr:Events/tr:Event[@ID='Error Message']/tr:Data/c:Datum/c:Value"/>
							</xsl:call-template>
						</xsl:if>
		...
		<xsl:if test="count(tr:Test|tr:SessionAction|tr:TestGroup)>0">
			<xsl:if test="not($gRemoveExpandCollapseFunctionality)">
				<span style="font-size:70%;font-weight:bold;">
					<img onclick="expandCollapseAll(event)" alt="Expand/Collapse" src="{$gMinusImage}" class="MainSequence_{$uniqueDivId}" id="MainSequence_{$uniqueDivId}img"/>
				...
				<xsl:if test="$nominalValueAttributes!=''">
					<tr class="hidden level:2 state:0">
						<td colspan="{$gHeadingSpan}" style="padding-left:{$gSingleSpaceValue * 2 - $gImageWidthValue}px; vertical-align:middle;">
							<img alt="" src="{$gPlusImage}" onclick="expandCollapseTR(event)" class="trExpanded"/>Nominal:</td>
					</tr>
		...
		<tr style="color:{$reportOptions/c:Item[@name='Colors']/c:Collection/c:Item[@name='Error']}">
			...
			<td colspan="{$colspan - 1}" style="white-space:normal;text-align:left;vertical-align:middle;">
				<xsl:if test="$eventsNode/tr:Event[@ID='Error Message']">
					<xsl:call-template name="ReplaceNewLineWithBreak">
						<xsl:with-param name="inputString" select="$eventsNode/tr:Event[@ID='Error Message']/tr:Data/c:Datum/c:Value"/>
					</xsl:call-template>
				</xsl:if>
	...
	<xsl:template name="AddTableElement">
		...
		<tr>
			...
			<td rowspan="2" valign="bottom" style="text-align:center;" width="5%">
				<b>Units</b>
			</td>
	...
	<xsl:template name="ProcessPropertyLoaderResult">
		...
		<xsl:if test="count(./c:Collection/c:Item[@name='ListOfUpdatedProperties']/c:Collection/c:Item)!=0">
			<tr class="{$visibility} level:{$numSpacesForListOfUpdatedProperties} state:0">
				<xsl:variable name="paddingLeftUpdatedProperties" select="$numSpacesForListOfUpdatedProperties * $gSingleSpaceValue"/>
				<td colspan="{$gHeadingSpan}" style="vertical-align:top; padding-left:{$paddingLeftUpdatedProperties}px;">
					<img alt="" src="{$gPlusImage}" onclick="expandCollapseTR(event)" class="trExpanded"/>
